...
main(){
  ...
  test('password should be 8 and more in length', (){
    ...
    expect(result, const Left(InvalidInputFailure([PasswordValidator.errorLength,PasswordValidator.errorLowercase,PasswordValidator.errorUppercase])));
  });
  test('password should have at least one number', (){
    String password = 'aaaAAAaaaAAAaAa';
    var result = passwordValidator.validate(password);
    expect(result, const Left(InvalidInputFailure(['Password should have at least one number.'])));
  });

  test('password should have at least one lowercase', (){
    String password = '1234AAAAAA';
    var result = passwordValidator.validate(password);
    expect(result, const Left(InvalidInputFailure(['Password should have at least one lowercase letter.'])));
  });
  test('password should have at least one uppercase', (){
    String password = '1234aaaaaaa';
    var result = passwordValidator.validate(password);
    expect(result, const Left(InvalidInputFailure(['Password should have at least one uppercase letter.'])));
  });
